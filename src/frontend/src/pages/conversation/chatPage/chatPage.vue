    ...
    try {
      abortCtrl.value = sendMessage(
        ...
        (msg: any) => {
          ...
          try {
            console.log('【原始消息】msg:', msg)
            console.log('【原始消息】msg.data:', msg.data)
            
            const parsedData = JSON.parse(msg.data)
            console.log('【解析后数据】parsedData:', parsedData)
            ...
            if (parsedData.type === 'response_chunk') {
              ...
              console.log('【Chunk累加后】aiMessage.content:', lastMessage.aiMessage.content)
              scrollBottom()
...
.event-info-row {
  ...
  &.END { 
    border-left: 4px solid var(--newsprint-accent); 
    background: var(--newsprint-neutral-100);
  }
...
.input-area {
  ...
  .action-btn {
    ...
    transition: all 0.3s ease;
    &:hover {
      background-color: var(--newsprint-border);
      color: var(--newsprint-fg);
      border-color: var(--newsprint-fg);
      transform: translate(-2px, -2px);
